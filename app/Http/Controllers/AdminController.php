...
class AdminController extends Controller
{
 ...
 public function guardarUsuario(Request $datos){
   ...
   /* Mail::send('admin.emails', ['user' => $user] ,function($message) use ($user) {
        $message->from('testsantillan@gmail.com','Tech Shop online store');
...
$data = [];
 Mail::send($config,$data, function($message) use($config)
          {
            $message->from('testsantillan@gmail.com');
            $message->to($config['email'],'usuario registrado');
            $message->subject('Bienvenidos Gracias por registrarse');
          });
